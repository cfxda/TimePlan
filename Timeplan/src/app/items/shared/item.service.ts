...
export class ItemService {
  ...
  items: FirebaseListObservable<any[]> = null; //  list of objects
    fItems: FirebaseListObservable<Item[]> = null; //  list of objects
  item: FirebaseObjectObservable<Item> = null; //   single object
  public groups = new vis.DataSet(); // list of groups
  ...
  public bgColor = '#127bdc';
  public fontColor = '#000';
  public brdColor = '#RRGGBBAA';
  public styleSet = [['blue', 'color:#f3f3f3;background-color:#273296; border-color:rgba(18,123,220,0); dot-color:#fff;border-left-color:#273296 ! important;'],
  ...
  getItemsList(query = {}): FirebaseListObservable<Item[]> {
   ...
   //add groups
   this.items.subscribe(items => items.forEach(item => {if (this.groupList.indexOf(item.group) == -1) {this.groupList.push(item.group)} }))
       
  return this.items
  }
  
//returns an array instead of a vis data list
